...
- **Dev runner:** `tsx` is used in development (`"dev": "tsx server.ts"`). Production build is `esbuild` → `dist/server.cjs` (CJS, external packages).
...
- **API keys:** Run through `normalizeOpenRouterApiKey` ([server.ts:5-8](server.ts#L5-L8)) before use. Never log full keys; never echo them to the client.
